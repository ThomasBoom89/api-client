...
func (c *CollectionClient) QueryRequests(co *Collection) *RequestQuery {
...
// QueryProject queries the project edge of a Collection.
func (c *CollectionClient) QueryProject(co *Collection) *ProjectQuery {
...
// QueryCollections queries the collections edge of a Project.
func (c *ProjectClient) QueryCollections(pr *Project) *CollectionQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, project.CollectionsTable, project.CollectionsColumn),
		)
...
func (c *RequestClient) QueryCollection(r *Request) *CollectionQuery {
	query := (&CollectionClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := r.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Collection, Project, Request []ent.Hook
	}
	inters struct {
		Collection, Project, Request []ent.Interceptor
	}
)
