...
func (cq *CollectionQuery) QueryRequests() *RequestQuery {
...
func (cq *CollectionQuery) QueryProject() *ProjectQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, collection.ProjectTable, collection.ProjectColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CollectionQuery) WithRequests(opts ...func(*RequestQuery)) *CollectionQuery {
	query := (&RequestClient{config: cq.config}).Query()
...
func (cq *CollectionQuery) WithProject(opts ...func(*ProjectQuery)) *CollectionQuery {
	query := (&ProjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(collection.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Collection.Query().
//		Select(collection.FieldCreateTime).
//		Scan(ctx, &v)
...
func (cq *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := cq.withRequests; query != nil {
		if err := cq.loadRequests(ctx, query, nodes,
			func(n *Collection) { n.Edges.Requests = []*Request{} },
			func(n *Collection, e *Request) { n.Edges.Requests = append(n.Edges.Requests, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withProject; query != nil {
		if err := cq.loadProject(ctx, query, nodes, nil,
			func(n *Collection, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CollectionQuery) loadRequests(ctx context.Context, query *RequestQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Request)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(request.FieldCollectionID)
	}
	query.Where(predicate.Request(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.RequestsColumn), fks...))
	}))
...
func (cq *CollectionQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Project)) error {
...
func (cq *CollectionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withProject != nil {
			_spec.Node.AddColumnOnce(collection.FieldProjectID)
		}
	}
